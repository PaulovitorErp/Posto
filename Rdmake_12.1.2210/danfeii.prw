...
If nTipo <> 1
	...
	If lIsLoja
		...
		//TBC-GO: Personalizacao: Ajuste de errorlog
...
//TBC-GO verifico se vem do Kingposto, e sem tem o XML gravado na MHQ
MHQ->(DbSetOrder(1)) //MHQ_FILIAL+MHQ_ORIGEM+MHQ_CPROCE+MHQ_CHVUNI+MHQ_EVENTO+DTOS(MHQ_DATGER)+MHQ_HORGER
if MHQ->(DbSeek(xFilial("MHQ")+PadR("KINGPOSTO",TamSX3("MHQ_ORIGEM")[1])+PadR("XML",TamSX3("MHQ_CPROCE")[1])+xFilial("SF2")+aIdNfe[nZ][4]+aIdNfe[nZ][5] ))

	aadd(aRetorno,{"","",aIdNfe[nZ][4]+aIdNfe[nZ][5],"","","",CToD(""),"","","",""})

	cRetorno	:= MHQ->MHQ_MENSAG
