...
/*/{Protheus.doc} TRETE042
Tela para multiselecao de Grupos de Produtos. Chamada pelo campo A1_XRESTGP
@author Totvs TBC
@since 18/07/2014
@version 1.0
...
@type function
/*/
User Function TRETE042()

Local nI
Local cQry			:= ""
Local aColunas 		:= StrTokArr("BM_GRUPO,BM_DESC",",")
Local aInf			:= {}
Local aDados		:= {}        
...
If Alltrim(ReadVar()) == "M->A1_XRESTGP"
	
	CursorWait()

	aInf := IIF(!Empty(M->A1_XRESTGP),StrTokArr(AllTrim(M->A1_XRESTGP),"/"),{})
	cInf := M->A1_XRESTGP
	...
	#ENDIF
		...
		Do While SBM->(!EOF()) .And. SBM->BM_FILIAL == xFilial(_cAlias)
			
			AAdd(aDados,{SBM->BM_GRUPO,SBM->BM_DESC})
	...
	DEFINE MSDIALOG oDlg TITLE "Seleção de Dados - Grupos de Produto" From 000,000 TO 450,700 COLORS 0, 16777215 PIXEL
...
While TRBAUX->(!EOF())
	If TRBAUX->OK == "mk"        
		If !lAux
			M->A1_XRESTGP := AllTrim(TRBAUX->COL1)
			lAux := .T.
		Else                             
			M->A1_XRESTGP += "/" + AllTrim(TRBAUX->COL1)
		Endif    
...
FWFldPut("A1_XRESTGP", M->A1_XRESTGP)
oView:Refresh()
...
User Function TR042VK1()
	SetKey(VK_F12, {|| U_TR042VK2() } )
Return .T.

User Function TR042VK2()
	If Alltrim(ReadVar()) == "M->A1_XRESTGP"
		U_TRETE042()
	elseif Alltrim(ReadVar()) == "M->A1_XRESTPR"
		U_TRETE043()
	endif
Return .T.
