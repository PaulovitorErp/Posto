...
User Function xCALLCHGXNU() //Afterlogin()
...
Static Function TIDevChild()
    ...
    If ! IsUsrAdmin() 
        ...
        //Return 
    ElseIf UsrRetName(__cUserId)  == "ATENDCONSULTA"
        __lAdmin := .f.     
    Else 
        __lAdmin := .t.     
    EndIf 
...
Static Function VldLogin(cUser, cSenha, cUserID)
    ...
    If nRetPsw == 0 .and. ! cUser == "ATENDCONSULTA"   //usuario admin
        lRet     := .T.
        __lAdmin := .T.
    Elseif nRetPsw == 2  //senha invalida 
        FWAlertWarning("Senha e/ou usuario invalidos!")
        lRet:= .f.
    Elseif nRetPsw == 1 .or. cUser == "ATENDCONSULTA"  // usuario não admin
