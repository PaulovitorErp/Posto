...
User Function LOJRNFCe(	oNFCe		, oProt		, nDecimais	, aFormas	,;
	...
	if oMyPrint <> Nil
		...
		oPrint := FWMsPrinter():New("Impressão " + iif( Type("lNFe") = "L" .and. lNFe, "NF-e_", "NFC-e_")+cChvNFCe, IMP_PDF, lAdjustToLegacy,cPath)
	endif
...
User Function LjRImpNFCE(	oNFCe		, oProt		, nDecimais	, aFormas	,; 
	...
	*/
	...
	oPrint:Say( nAuxLn, aColDet[01, 02], PadC("DOCUMENTO AUXILIAR DA NOTA FISCAL DE CONSUMIDOR ELETRÔNICA", 85) )
	
	/* todo aumentar a fonte*/ 
	If lContigen
		nAuxLn += 60
		oPrint:Say( nAuxLn,aColDet[01, 02], PadC("EMITIDA EM CONTINGÊNCIA", 66), oFont4 )
		nAuxLn += 40
		oPrint:Say( nAuxLn,aColDet[01, 02], PadC("Pendente de autorização", 85), oFont )
	EndIf
	...
	*/	
	...
	If cAmbiente == "2"		
		oPrint:Say( nAuxLn, aColDet[01, 02], PadC("EMITIDA EM AMBIENTE DE HOMOLOGAÇÃO – SEM VALOR FISCAL",85), oFont )
		nAuxLn += 60
	EndIf
	...
	*/
	...
	//TBC: modificado para não ficar muito espaço
	nAuxLn += 400// 775
	...
	*/
	If Type("oNfce:_NFE:_INFNFE:_INFADIC") == "O" .AND. LjRTemNode(oNfce:_NFE:_INFNFE:_INFADIC, "_INFCPL")
		...
		For nY := 1 to nInfCpl
			...
			//TBC-GO ajustado para não perder informações quando o texto nao cabe na linha
...
Local nTotDeson     := 0	// Valor Total do imposto da desoneração
Local aMsgNfPre     := {}   // Mensagem de Nota Fiscal Premiada
Local lImpDnf		:= SuperGetMV("MV_XIMPDNF",,.F.) //Habilita pergunta de impressão de danfinha (default .F.)
...
/*TBC: ajuste a opção de imprimir ou não a nota ao finalizar a venda.*/
If lImpDnf .AND. !lNFe
	if !MsgYesNo("Deseja imprimir a danfinha?", "Atenção")
		lSaiImp := .T.
	endif
Elseif lNFe
	nX := Aviso("NFe", "NF-e Autorizada. Como quer a impressão da DANFE?", {"DANFE A4", "Não Imprimir", "Cupom"}, 2)
	if nX == 2
		lSaiImp := .T.
	elseif nX == 1
		LjGrvLog( SL1->L1_NUM," Realiza a impressão do DANFE A4",  )
...
BEGIN SEQUENCE
	...
	//-----------------------------------------------------
	aRet := LjXMLNFCe(cXML) //-- TBC-GO Validação do retorno da função LjXMLNFCe
	...
	//-- TBC-GO Tratamento para evitar erro na impressão...
	If ValType(oNFCe) <> "O" //aborta a impressao
		MsgStop("NFC-e: Não foi possível obter o XML da NFC-e")
		BREAK
	ElseIf Type("oNfce:_NFE:_SIGNATURE:_SIGNEDINFO:_REFERENCE:_DIGESTVALUE") <> "O" //Validação do 'Digest Value'
		MsgStop("NFC-e: Não foi possível obter o XML da NFC-e. Erro: Digest Value")
		BREAK
	Else
...
TBC-GOCarrega as formas de pagamento padrão 
...
//TBC-GO (copiado parte da função do fonte LONNFCE)
...
If lRetAux
	...
	LjGrvLog( SL1->L1_NUM," Realiza a impressão do DANFE",  )
